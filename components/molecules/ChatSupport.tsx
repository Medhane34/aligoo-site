// components/organism/ChatSupportWidget.tsx
"use client";
...
export default function ChatSupportWidget() {
  ...
  /**
   * Handles WhatsApp button click with Google Analytics tracking
   * Fires before opening WhatsApp chat
   */
  const handleWhatsAppClick = useCallback(() => {
    // === GOOGLE ANALYTICS EVENT TRACKING ===
    if (typeof window !== "undefined" && (window as any).gtag) {
      (window as any).gtag("event", "whatsapp_chat_click", {
        event_category: "contact",
        event_label: "floating_widget",
        source: "chat_support_fab",
        language: "en", // You can make this dynamic if you support multiple languages
        button_text: "Chat on WhatsApp",
        page_location: window.location.pathname,
        page_title: document.title,
        debug_mode: process.env.NODE_ENV === "development", // Auto-debug in dev
      });
    } else {
      console.warn("gtag not available - WhatsApp click event not sent");
    }
    // ======================================

    // Original WhatsApp redirect logic
                  ...
                  <div className="p-6 bg-neutral-900/50">
                    <div className="bg-white/5 rounded-xl p-4 rounded-tl-none border border-white/5 mb-6">
                      <p className="text-neutral-200 text-sm leading-relaxed">
                        Hi there! 👋 <br />
                        <br />
                        Thanks for visiting Aligoo. How can we help you grow your
                        business today?
                      </p>
                    ...
                    {/* WhatsApp Button - NOW WITH GA TRACKING */}
                    <button
