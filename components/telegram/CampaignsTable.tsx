...
export default function CampaignsTable({
  ...
  const [serviceFilter, setServiceFilter] = useState<Set<string> | "all">(
    "all",
  );
  ...
  const sortedItems = useMemo(() => {
    return [...filteredItems].sort((a: any, b: any) => {
      const first =
        sortDescriptor.column === "stats.sent"
          ? a.stats?.sent
          : sortDescriptor.column === "stats.failed"
            ? a.stats?.failed
  ...
  const renderCell = (campaign: Campaign, columnKey: string) => {
    switch (columnKey) {
      case "title":
        return (
          <span className="font-semibold text-white/90">{campaign.title}</span>
        );
      case "service":
        return (
          <BadgeAtom
      ...
      case "status":
        ...
        return (
          <BadgeAtom
            color={statusColors[campaign.status] || "blue"}
            variant="filled"
          >
            {campaign.status}
          </BadgeAtom>
      ...
      case "stats.sent":
        return (
          <span className="text-white/70">{campaign.stats?.sent || 0}</span>
        );
      case "stats.failed":
        return (
          <span className="text-white/70">{campaign.stats?.failed || 0}</span>
        );
      case "sentAt":
        return (
