      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20">
          ...
          >
            <BrowserFrame
              height="h-[600px]"
              isHovered={hoveredProject === projects[0].id}
              project={projects[0]}
            />
          </motion.div>
...
function BrowserFrame({
  ...
  return (
    <div
      className={`relative w-full ${height} rounded-2xl bg-neutral-900 border border-white/10 overflow-hidden shadow-2xl transition-all duration-500 ${isHovered ? "shadow-[#FF595E]/20 scale-[1.02]" : ""}`}
    >
      {/* Browser Header */}
      ...
      <div className="absolute inset-0 pt-10 bg-neutral-900 overflow-hidden">
        ...
        <div
          className={`w-full h-[200%] bg-gradient-to-br ${project.color} opacity-20 transition-transform duration-[3s] ease-in-out ${isHovered ? "-translate-y-1/4" : "translate-y-0"}`}
        />

        {/* Actual Image Container (Simulating Scroll) */}
        <div
          className={`absolute inset-0 pt-10 transition-transform duration-[2s] ease-in-out ${isHovered ? "-translate-y-20" : "translate-y-0"}`}
        >
          <Image
            fill
            alt={project.name}
