...
import { Check, Flag, BarChart3, Rocket, Layers } from "lucide-react";
import { PortableText } from "@portabletext/react";

import { RoadmapSection as RoadmapType } from "../../types/ProposalType";
    ...
    <section className="py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16 max-w-3xl mx-auto">
          ...
          >
            {data.subheading ||
              "We don't just sell services. We build systems that scale with your business."}
          </motion.p>
        </div>
            ...
            {data.phases.map((phase, index) => (
              ...
              >
                ...
                <div className="hidden md:flex absolute -top-10 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-slate-900 border-4 border-slate-800 items-center justify-center z-10 group-hover:border-white/20 transition-colors">
                  <div
                    className={`w-2 h-2 rounded-full bg-gradient-to-r ${getPhaseColor(phase.color)}`}
                  />
                </div>
                ...
                {/* Card */}
                <div
                  className={`h-full relative overflow-hidden rounded-2xl border ${getPhaseBorder(phase.color)} hover:bg-slate-800/50 transition-all duration-300 backdrop-blur-sm p-6 flex flex-col`}
                >
                  {/* Phase Badge */}
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getPhaseColor(phase.color)} p-[1px]`}
                      >
                      ...
                      <div>
                        <div
                          className={`text-xs font-bold uppercase tracking-wider bg-clip-text text-transparent bg-gradient-to-r ${getPhaseColor(phase.color)}`}
                        >
                          {phase.duration}
                        </div>
                        <h3 className="text-xl font-bold text-white leading-none mt-1">
                          {phase.title}
                        </h3>
                      </div>
                    </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-white/5">
                    <div className="flex items-start gap-3">
                      <div
                        className={`mt-1 p-1 rounded-full bg-gradient-to-r ${getPhaseColor(phase.color)} opacity-80`}
                      >
