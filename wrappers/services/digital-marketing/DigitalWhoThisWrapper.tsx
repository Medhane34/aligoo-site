import { fetchWhoThisIsForSection, WhoThisIsForSectionData } from "@/lib/services/digitalmarketing";
import WhoThisIsForSection from "@/components/service-sections/WhoThisIsFor";
export default async function DigitalWhoThisIsForSectionWrapper({
  lang = "en",
}: {
  lang?: "en" | "am";
}) {
  const data: WhoThisIsForSectionData | null = await fetchWhoThisIsForSection("whoThisIsForSection-Digital");
  if (!data) return null;
  return (
    <WhoThisIsForSection
