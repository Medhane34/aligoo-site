...
export function getProposalStatus(proposal: DashboardProposal): {
  text: ProposalStatus;
  color: string;
} {
  const now = new Date();
  ...
  if (
    proposal.currentSelection &&
    proposal.currentSelection?.status === "Accepted"
  )
    return { text: "Accepted", color: "bg-green-500" };
  if (
    proposal.currentSelection &&
    proposal.currentSelection?.status === "Rejected"
  )
    return { text: "Rejected", color: "bg-red-500" };
  if (
    proposal.currentSelection &&
    proposal.currentSelection?.status === "Payment_pending"
  )
