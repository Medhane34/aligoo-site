...
import { updateProposalSelection } from "@/lib/proposal";
...
export default function PricingTableWrapper({
  ...
  if (
    comparisonTable?.enabled === false ||
    !comparisonTable ||
    !packagePricing ||
    !comparisonTable.groups ||
    comparisonTable.groups.length === 0
  ) {
  ...
  const [selectedPackage, setSelectedPackage] =
    useState<PackageKey>(initialPackage);
  ...
  useEffect(() => {
    const saveInitialSelection = async () => {
      ...
      if (
        ...
        comparisonTable.recommendedPackage
      ) {
        try {
          await updateProposalSelection(
            proposalId,
            initialPackage,
            selectedAddOns,
          );
