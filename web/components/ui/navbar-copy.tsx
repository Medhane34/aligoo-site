...
export function Navbar() {
  ...
  return (
    <>
      {/* Desktop Header */}
      <header
        className={`fixed top-4 left-1/2 -translate-x-1/2 z-[9999] mx-auto hidden w-full flex-row items-center justify-between self-start rounded-full bg-white/80 dark:bg-[#0b0b10]/80 md:flex backdrop-blur-sm border border-black/5 dark:border-white/10 shadow-lg transition-all duration-300 ${isScrolled ? "max-w-3xl px-2" : "max-w-5xl px-4"
          } py-2`}
      ...
      >
        ...
        {/* Mega Menu Panel — centered on the navbar pill */}
        <AnimatePresence>
          {isServicesMenuOpen && (
            <motion.div
              ...
              className="absolute left-1/2 top-full w-[min(1152px,calc(100vw-2rem))] -translate-x-1/2"
              onBlur={(e) => {
            ...
            >
              <div className="mt-2 overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 bg-white shadow-2xl ring-1 ring-black/5 dark:bg-[#0b0b10]">
                ...
                >
                  {serviceGroups.map((group, groupIndex) => (
                    <motion.div
                      ...
                      className={groupIndex > 0 ? "md:border-l md:border-black/10 md:pl-10 dark:border-white/10" : ""}
                    >
                      <h3 className="text-lg font-bold text-neutral-900 dark:text-white">{group.title}</h3>
                      ...
                      <ul className="mt-5 space-y-1">
                        {group.items.map((item) => (
                          <li key={item.href}>
                            <Link
                              className="group flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors duration-200 hover:bg-black/5 dark:hover:bg-white/5"
                              href={`/${lang}${item.href}`}
                            >
                              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-neutral-100 text-neutral-700 shadow-sm transition-all duration-200 group-hover:bg-gradient-to-r group-hover:from-[#FF595E] group-hover:to-orange-500 group-hover:text-white dark:bg-white">
      ...
      {/* Mobile Header */}
      <header className="fixed top-4 z-[9999] left-4 right-4 mx-auto flex w-auto flex-row items-center justify-between rounded-full bg-white/80 dark:bg-[#0b0b10]/80 backdrop-blur-sm border border-black/5 dark:border-white/10 shadow-lg md:hidden px-4 py-3">
        <ThemeSwitch />
        ...
        <button
          ...
          className="flex items-center justify-center w-10 h-10 rounded-full bg-white/50 dark:bg-white/10 border border-black/5 dark:border-white/10 transition-colors hover:bg-white/80 dark:hover:bg-white/20"
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[9998] bg-black/50 backdrop-blur-sm md:hidden">
          <div className="absolute top-20 left-4 right-4 bg-white dark:bg-[#0b0b10] backdrop-blur-md border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl p-6">
                      ...
                      <div key={group.title}>
                        ...
                        <div className="flex flex-col">
                          {group.items.map((item) => (
                            <Link
                              key={item.href}
                              className="text-base text-neutral-500 hover:text-primary transition-colors py-1 dark:text-neutral-400"
                              href={`/${lang}${item.href}`}
                              onClick={() => setIsMobileMenuOpen(false)}
                            >
                              {item.label}
