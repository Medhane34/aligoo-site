  ...
  return (
    <>
          ...
          >
            ...
            <div
              className={`relative overflow-hidden rounded-3xl shadow-2xl transition-all ${
                selectedPackage === pkg
                  ? "ring-4 ring-white/60 shadow-yellow-500/60"
                  : ""
              }`}
            >
              {/* Gradient Background */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${packageColors[pkg]} opacity-90`}
              />
      ...
      <div className="md:hidden mt-12 text-center">
        ...
        >
          Select {packageLabels[currentViewPackage]} Package —{" "}
          {formatPrice(packagePricing[currentViewPackage])}
        </button>
      </div>
    </>
  );
