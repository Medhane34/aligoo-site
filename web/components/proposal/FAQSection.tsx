...
function AccordionItem({ faq, index }: { faq: FAQItem; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`text-lg font-bold transition-colors ${isOpen ? "text-white" : "text-neutral-300"}`}
        >
          {faq.question}
        </span>
        <div
          className={`p-2 rounded-full transition-colors ${isOpen ? "bg-[#FF595E] text-white" : "bg-white/10 text-neutral-400"}`}
        >
          {isOpen ? (
