...
const bot = new Bot(process.env.TELEGRAM_CLIENT_BOT_TOKEN as string)
  ...
  .on("message", async (context) => {
    ...
    if (context.text === "✅ Confirm Subscription") {
      ...
      await context.send("You're all set! 🎉\n\n Move our bot to personal folder or pin so you don't miss any new updates.");
  ...
  .on("callback_query", async (context) => {
    ...
    if (callbackData.startsWith("service_")) {
      ...
      if (serviceId === "done") {
        ...
        return context.send(
          "One last step. Please confirm you want to receive our marketing updates.",
          {
            reply_markup: {
      ...
      try {
        if (context.message) {
          await context.message.editReplyMarkup(
            getServiceKeyboard(newServices),
          );
        }
      } catch (e) {
