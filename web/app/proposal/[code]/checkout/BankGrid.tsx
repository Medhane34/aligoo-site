...
const BankLogo = ({ name }: { name: string }) => {
  ...
  if (name.toLowerCase().includes("telebirr"))
    return <Smartphone className="w-8 h-8 text-cyan-400" />;
  if (name.toLowerCase().includes("cbe"))
    return <Building2 className="w-8 h-8 text-purple-400" />;
            ...
            {banks[selectedBank].accountNumber && (
              <div className="col-span-full bg-black/30 p-4 rounded-lg border border-white/5 flex items-center justify-between group">
                <div>
                  ...
                  <p className="font-mono text-xl text-cyan-400 tracking-wide">
                    {banks[selectedBank].accountNumber}
                  </p>
                </div>
                <button
                  ...
                  onClick={() =>
                    handleCopy(banks[selectedBank].accountNumber!, selectedBank)
                  }
                >
