...
const MOCK_POSTS: BlogPost[] = [
  {
    ...
    excerpt:
      "A step-by-step breakdown of our viral campaign that turned a small Addis café into a city sensation using hyper-targeted TikTok ads and UGC.",
  ...
  },
  {
    ...
    excerpt:
      "Common UX mistakes Ethiopian businesses make — and how a 3-second load time can double your conversions.",
  ...
  ...Array.from({ length: 12 }, (_, i) => ({
    ...
    title:
      i % 2 === 0
        ? `SEO Strategy #${i + 1}`
        : `Content Ideas for ${SERVICES[i % 8].title}`,
    excerpt:
      "Mock post to test pagination, filtering, and responsive grid. Includes long text to test line clamping and card overflow.",
    slug: `mock-${i + 1}`,
    imageUrl:
      i % 3 === 0
        ? undefined
        : "https://images.unsplash.com/photo-1517180105933-7b30e3a0d67d?w=800",
...
export const fetchBlogPosts = async (
  ...
  let filtered = service
    ? MOCK_POSTS.filter((p) => p.serviceTitle === service)
    : MOCK_POSTS;
...
export const fetchTotalBlogPostsCount = async (
  ...
  return service
    ? MOCK_POSTS.filter((p) => p.serviceTitle === service).length
    : MOCK_POSTS.length;
};
