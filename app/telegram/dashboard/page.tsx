...
async function getStats() {
  const subscribers = await automationClient.fetch(
    `count(*[_type == "subscriber"])`,
  );
  const activeSubscribers = await automationClient.fetch(
    `count(*[_type == "subscriber" && isActive == true])`,
  );
  const campaigns = await automationClient.fetch(
    `*[_type == "campaign"]{status, stats}`,
  );
  ...
  const deliveryRate =
    totalAttempts > 0 ? ((totalSent / totalAttempts) * 100).toFixed(1) : "0";
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-default-500">
          Real-time metrics for your Telegram marketing.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <CardMolecule className="flex flex-col gap-2" variant="spotlight">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-xs text-success">
              {stats.activeSubscribers} Active Users
            </p>
          </div>
        </CardMolecule>
