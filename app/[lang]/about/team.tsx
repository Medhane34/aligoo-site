    ...
    <section className="py-20 px-4 bg-background-light dark:bg-background-dark">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <SectionHeading
            className={`text-heading uppercase mb-4 ${isAmharic ? "font-amharicHeading text-5xl" : ""}`}
          >
            {heading}
          </SectionHeading>
          <AccentText
            className={`text-lg ${isAmharic ? "font-amharicBody text-2xl leading-relaxed" : ""}`}
          >
            {subheading}
          </AccentText>
        </div>
          ...
          {members.map((person, i) => {
            ...
            return (
              ...
              >
                {person.department && (
                  <span
                    className={`mb-6 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider ${person.departmentColor || "bg-yellow-400 text-black"}`}
                  >
                    {person.department}
                  </span>
                )}
                ...
                <h3 className="text-2xl font-bold mb-2 text-center">
                  <span className={person.firstNameColor || "text-yellow-400"}>
                    {firstName}
                  </span>{" "}
                  <span className={person.lastNameColor || "text-white"}>
                    {lastName.join(" ")}
                  </span>
                </h3>
                ...
                <p
                  className={`text-gray-300 text-center text-sm leading-relaxed ${isAmharic ? "font-amharicBody text-lg" : ""}`}
                >
                  {person.bio}
                </p>
              </motion.article>
