...
export async function POST(req: Request) {
  try {
    ...
    const imageAsset = await automationClient.assets.upload(
      "image",
      proofFile,
      {
    ...
    console.log(
      "CLIENT_BOT_TOKEN exists:",
      !!process.env.TELEGRAM_CLIENT_BOT_TOKEN,
    );
    console.log(
      "CLIENT_CHAT_ID exists:",
      !!process.env.TELEGRAM_CLIENT_CHAT_ID,
    );
    console.log(
      "Token preview (first 20 chars):",
      process.env.TELEGRAM_CLIENT_BOT_TOKEN?.slice(0, 20),
    );
    ...
    if (
      ...
      return NextResponse.json({
        success: true,
        warning: "Client bot not configured",
      });
    }
