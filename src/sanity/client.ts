...
sanityClient.fetch = async <R = any>(
  ...
  try {
    ...
    console.warn(
      "⚠️ Sanity Fetch Failed (Likely Quota/Network). Switching to Fallback Data.",
      error.message,
    );
    ...
    if (queryStr.includes('"heroSection"') && params.name) {
      ...
      if (
        nameStr.includes("Content") ||
        nameStr.includes("content") ||
        nameStr.includes("Marketing") ||
        nameStr.includes("marketing")
      ) {
    ...
    if (queryStr.includes('"heroSection"') && !params.name)
      return FALLBACK_HERO as unknown as R;
    if (queryStr.includes('"aboutUsSection"'))
      return FALLBACK_ABOUT as unknown as R;
    if (queryStr.includes('"statsSection"'))
      return getFallbackStats(lang) as unknown as R;
    if (queryStr.includes('"serviceSection"'))
      return getFallbackServices(lang) as unknown as R;
    if (queryStr.includes('"processSection"'))
      return getFallbackProcess(lang) as unknown as R;
    if (queryStr.includes('"whyUsSection"'))
      return getFallbackWhyUs(lang) as unknown as R;
    if (queryStr.includes('"testimonial"'))
      return getFallbackTestimonials(lang) as unknown as R;
    if (queryStr.includes('"ctaSection"')) return FALLBACK_CTA as unknown as R;

    // -- About Page Sections (with language support) --
    if (queryStr.includes('"aboutIntroSection"'))
      return getFallbackAboutIntro(lang) as unknown as R;
    if (queryStr.includes('"valuesSection"'))
      return getFallbackValues(lang) as unknown as R;
    if (queryStr.includes('"teamSection"'))
      return getFallbackTeam(lang) as unknown as R;
    if (queryStr.includes('"meaningSection"'))
      return getFallbackMeaning(lang) as unknown as R;
    if (queryStr.includes('"ourWaySection"'))
      return getFallbackOurWay(lang) as unknown as R;

    // -- Contact Page --
    if (queryStr.includes('"contactSection"'))
      return FALLBACK_CONTACT as unknown as R;

    // -- Facebook Ad Service Sections --
    if (
      queryStr.includes('"whyServiceWorksSection"') &&
      params.name?.includes("Fb")
    ) {
      return getFallbackFbWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
      queryStr.includes('"adPhilosophySection"') &&
      (params.name?.includes("Fb") || params.name?.includes("facebook"))
    ) {
      return getFallbackFbAdPhilosophy(params.name as string) as unknown as R;
    }
    if (
      queryStr.includes('"whoThisIsForSection"') &&
      (params.name?.includes("Fb") || params.name?.includes("facebook"))
    ) {
      return getFallbackFbWhoThisIsFor(params.name as string) as unknown as R;
    }
    if (
      queryStr.includes('"faqSection"') &&
      (params.name?.includes("Fb") || params.name?.includes("facebook"))
    ) {
    ...
    // Hero handled at top
    if (
      queryStr.includes('"ourProcessSection"') &&
      (params.name?.includes("Web") || params.name?.includes("web"))
    ) {
    ...
    if (
      queryStr.includes('"whyServiceWorksSection"') &&
      (params.name?.includes("TikTok") ||
        params.name?.includes("Tiktok") ||
        params.name?.includes("tiktok"))
    ) {
      return getFallbackTiktokWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
    ...
    if (
      ...
      return getFallbackTiktokWhoThisIsFor(
        params.name as string,
      ) as unknown as R;
    }
    if (
    ...
    if (
      queryStr.includes('"whyServiceWorksSection"') &&
      (params.name?.includes("Seo") ||
        params.name?.includes("seo") ||
        params.name?.includes("SEO"))
    ) {
      return getFallbackSeoWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
    ...
    if (
      queryStr.includes('"whyServiceWorksSection"') &&
      (params.name?.includes("Content") ||
        params.name?.includes("content") ||
        params.name?.includes("Marketing") ||
        params.name?.includes("marketing"))
    ) {
      return getFallbackContentMarketingWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackContentMarketingOurProcess(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackContentMarketingWhoThisIsFor(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackContentMarketingFaq(
        params.name as string,
      ) as unknown as R;
    }
    ...
    if (
      queryStr.includes('"whyServiceWorksSection"') &&
      (params.name?.includes("Digital") || params.name?.includes("digital"))
    ) {
      return getFallbackDigitalMarketingWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackDigitalMarketingOurProcess(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackDigitalMarketingWhoThisIsFor(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackDigitalMarketingFaq(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackDigitalMarketingCta(
        params.name as string,
      ) as unknown as R;
    }
    ...
    // Hero handled at top
    if (
      queryStr.includes('"whyServiceWorksFeatures"') &&
      (params.name?.includes("Funnel") || params.name?.includes("funnel"))
    ) {
      return getFallbackFunnelMappingWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackFunnelMappingOurProcess(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackFunnelMappingWhoThisIsFor(
        params.name as string,
      ) as unknown as R;
    }
    if (
    ...
    if (
      queryStr.includes('"whyServiceWorksFeatures"') &&
      (params.name?.includes("Graphic") || params.name?.includes("graphic"))
    ) {
      return getFallbackGraphicDesignWhyServiceWorks(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackGraphicDesignOurProcess(
        params.name as string,
      ) as unknown as R;
    }
    if (
      ...
      return getFallbackGraphicDesignWhoThisIsFor(
        params.name as string,
      ) as unknown as R;
    }
    if (
    ...
    if (queryStr.includes('"caseStudy"')) {
      ...
      if (queryStr.includes("[0]"))
        return FALLBACK_CASE_STUDIES.casestudyPosts[0] as unknown as R;
    ...
    if (queryStr.includes('"post"') || queryStr.includes('"blogPost"')) {
      if (queryStr.includes("[0]"))
        return FALLBACK_BLOG_POSTS[0] as unknown as R;
